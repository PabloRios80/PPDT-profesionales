...
if (response.data.status === 'success') {
    const { dni, nombre, apellido, especialidad, matricula, telefono, email } = req.body;
...
app.post('/api/profesionales/derivar', async (req, res) => {
    try {
        ...
        if (response.data.status === 'success') {
            const { dni, nombre, apellido, fechaNacimiento, telefono, email, observaciones, medicoDerivador } = req.body;
            ...
            if (error) console.error('ERROR SUPABASE derivaciones:', JSON.stringify(error));
